...
export default function BoxOfficeTbody({ dailyList, setSelMv }) {
  ...
  const tags = dailyList.map(item =>
    <tr key={item.movieCd} 
      ...
      <td className="whitespace-nowrap px-6 py-2 text-right">{parseInt(item.salesAcc).toLocaleString()}</td>
      <td className="whitespace-nowrap px-6 py-2 text-right">{parseInt(item.audiAcc).toLocaleString()}</td>
